...
[Word Sprint](https://github.com/) 主站的管理后台 —— 用主站账号登录后，可以表格化管理当前账号自己的单词分类、单词、笔记、口语分类、口语表达，并查看 AI 用量统计。

> 当前后端 API 是按登录用户过滤的（每个账号只看自己的数据）。这是有意为之，与主站 server 完全解耦，不需要改 server。
...
打开 http://localhost:5174 ，用你在主站注册过的用户名 / 密码登录。
